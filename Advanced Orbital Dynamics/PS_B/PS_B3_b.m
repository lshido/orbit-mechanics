...
for row = 1:rows
    ...
    while 1
        ...
        if abs(f) > tolerance
            ...
            all_results(end+1,:) = [name mu gamma gamma*a gamma*100 x x*a accel partial];
            counter = 0;
            break
        end
    end
end
t = array2table(all_results,'VariableNames',{'System', 'mu of system', 'gamma', 'gamma_dim', 'percentage of a', 'x', 'x_dim', 'Acceleration', 'Partial'});
format shortE
disp(t)