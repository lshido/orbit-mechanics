...
mean_motion_Asteroid = (7/3)*mean_motion_Jupiter;
...
%=======================Plot the orbits in the inertial frame=====================
...
title({'Orbits of Jupiter, Asteroid A, and Comet C in the inertial frame';['with e=',num2str(e),' and 3:5 resonance between the Comet and Jupiter']})
xlim([-2.1e9 2.1e9])
ylim([-2.1e9 2.1e9])
axis square
...
fig_anim_rot = figure('Name', 'Rotating Frame Animation!');
sun = scatter(x_Sun, y_Sun, 'red', 'filled', 'SizeData', 200);
hold on
...
%=============ANIMATE THE POSITIONS (rotating)!==========================
...
for i = 0:0.02:t_final
    ...
    TM = [cos(i*2*pi) sin(i*2*pi); -sin(i*2*pi) cos(i*2*pi)];
    comet_rotational = TM*[x_comet; y_comet];
    comet_marker.XData = comet_rotational(1);
    comet_marker.YData = comet_rotational(2);
    ...
    y_asteroid = b_asteroid * sin(theta_asteroid);
    asteroid_rotational = TM*[x_asteroid; y_asteroid];
    asteroid_marker.XData = asteroid_rotational(1);
    asteroid_marker.YData = asteroid_rotational(2);
    drawnow
    % exportgraphics(gca,"parabola.gif",Append=true)
end