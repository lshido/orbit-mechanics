ps = "D4 part c"
# Continuation Algorithm: Natural Parameter Process using JC
# Problem D4 Part c
...
def calc_vy_from_Jacobi(mu, x, y, vx, JC):
    d = sqrt((x+mu)**2 + y**2)
    r = sqrt((x-1+mu)**2 + y**2)
    x_y_sq = (x**2+y**2)/2
    term_1 = (1-mu)/d
...
for orbit in range(11):
    ...
    if orbit <= 1: # For the first two calculations, naively guess ydot is the same
        ...
        # Calc the slope from the last pair of x0 and jacobi
        my_slope = (df_orbits.iloc[-1]['jacobi']-df_orbits.iloc[-2]['jacobi'])/(df_orbits.iloc[-1]['x']-df_orbits.iloc[-2]['x'])
        jacobi = calc_Jacobi(mu, converged_initial_states[0], converged_initial_states[1], converged_initial_states[2], converged_initial_states[3])
        delta_JC = delta_x*my_slope
        # Calc Jacobi
        jacobi = jacobi + delta_JC
        pdb.set_trace()
        orbit_ydot = calc_vy_from_Jacobi(mu, orbit_x, converged_initial_states[1], converged_initial_states[2], jacobi)
        print(f"found x0:{converged_initial_states[0]}, found ydot: {converged_initial_states[3]}")
        # Compile data
        orbit_IC_data = pd.DataFrame({
            "orbit":[orbit],
