...
% Calculate the long linear orbit for the equilateral points:
...
for x = linspace(0,1,5e3) %  Find the curve for -1.5 < x < 1.5
    fprintf("x sweep: %f\n", x)
...
for y = linspace(0.5,1.1,5e3) % Find the curve for 0 < y < 1.21
    fprintf("y sweep: %f\n", y)
    if y == 0.752505
        fprintf("y = 0.752505!!\n")
    end
...
% zvc = plot(zvc_table, 'x', 'y');
nonlinear_orbit = plot(x_nl, y_nl, 'Color', '#008000');
hold off
% xlim([0.3 0.65])
% ylim([0.75 1.1])
xlim([0.46 0.52])
...
xlabel("x [non-dim]")
ylabel("y [non-dim]")
legend([L4_plot, zvc, linear_orbit, nonlinear_orbit], {'L4', 'ZVC', 'Linear Orbit', 'Non-linear Orbit'})
title({['L4 long period (x-y), \xi_0=', num2str(xi_0)]; ['JC=', num2str(C),', t=',num2str(t_end),'rad (Lillian Shido)']})
box on
grid on
fontsize(14, 'points')