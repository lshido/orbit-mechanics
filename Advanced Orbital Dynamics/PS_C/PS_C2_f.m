...
function [lambda_1, lambda_2, lambda_3, lambda_4] = calc_eigenvalues(mu, x_L, y_L)
...
function [x_L1, y_L1] = calc_L1(mu, a, gamma, tolerance) % default gamma = 0.2, tolerance = 1e-12
    L1_results = zeros(0,1);
    counter = 0;
    while 1
        counter = counter + 1;
        % Step 2
...
function [x_L2, y_L2] = calc_L2(mu, a, gamma, tolerance) % default gamma = 1e-6, tolerance = 1e-12
    L2_results = zeros(0,1);
    counter = 0;
    while 1
        counter = counter + 1;
        % Step 2
...
% Calculate the location of L3
function [x_L3, y_L3] = calc_L3(mu, a, gamma, tolerance) % default gamma = 1e-6, tolerance = 1e-12
    L3_results = zeros(0,1);
    counter = 0;
    while 1
        counter = counter + 1;
        % Step 2
...
% Calculate the location of L5
function [x_L5, y_L5] = calc_L5(mu)
    x_L5 = cosd(60) - mu;
    y_L5 = -sind(60);
end
...
% Calculate Jacobi Constant C with ICs
function C = calc_JC_from_ICs(mu, x_0, y_0, v_x_0, v_y_0)
    d_0 = sqrt((x_0+mu)^2 + y_0^2);
    r_0 = sqrt((x_0-1+mu)^2 + y_0^2);
    pseudo_U = (1-mu)/d_0 + mu/r_0 + (x_0^2+y_0^2)/2;
    v_squared = v_x_0^2 + v_y_0^2;
...
% Location of the Earth-Moon Libration points
[x_L1, y_L1] = calc_L1(mu, a, 0.2, 1e-12);
fprintf("L1 x: %f, y: %f\n", x_L1, y_L1)
[x_L2, y_L2] = calc_L2(mu, a, 1e-6, 1e-12);
% x_L2 = 1.1557e+00;
fprintf("L2 x: %f, y: %f\n", x_L2, y_L2)
[x_L3, y_L3] = calc_L3(mu, a, 1e-6, 1e-12);
% x_L3 = -1.0051;
fprintf("L3 x: %f, y: %f\n", x_L3, y_L3)
[x_L4, y_L4] = calc_L4(mu);
fprintf("L4 x: %f, y: %f\n", x_L4, y_L4)
[x_L5, y_L5] = calc_L5(mu);
fprintf("L5 x: %f, y: %f\n", x_L5, y_L5)
...
x_0 = x_L1 + xi_0;
y_0 = y_L1 + eta_0;
distance_from_moon = x_Moon-x_0;
fprintf("Distance from the Moon: %f [km]\n", distance_from_moon*l_char)
% WRT L1 frame [dimensional]
xi_0_dim = xi_0*l_char;
fprintf("Dimensional xi_0: %f km\n", xi_0_dim)

% Calculate the roots of the L1 point
[lambda_1, lambda_2, lambda_3, lambda_4] = calc_eigenvalues(mu, x_L1, y_L1);
...
% Calculate the roots of the L2 point
[L2_lambda_1, L2_lambda_2, L2_lambda_3, L2_lambda_4] = calc_eigenvalues(mu, x_L2, y_L2);
fprintf("L2_lambda_1: %f + %fi\n", real(L2_lambda_1), imag(L2_lambda_1))
fprintf("L2_lambda_2: %f + %fi\n", real(L2_lambda_2), imag(L2_lambda_2))
fprintf("L2_lambda_3: %f + %fi\n", real(L2_lambda_3), imag(L2_lambda_3))
fprintf("L2_lambda_4: %f + %fi\n", real(L2_lambda_4), imag(L2_lambda_4))

% Calculate the roots of the L3 point
[L3_lambda_1, L3_lambda_2, L3_lambda_3, L3_lambda_4] = calc_eigenvalues(mu, x_L3, y_L3);
fprintf("L3_lambda_1: %f + %fi\n", real(L3_lambda_1), imag(L3_lambda_1))
fprintf("L3_lambda_2: %f + %fi\n", real(L3_lambda_2), imag(L3_lambda_2))
...
v_y0_dim = v_eta_0 * l_char / t_char * 1000;
...
x_0 = x_L1 + xi_0;
y_0 = 0;
C = calc_JC_from_ICs(mu, x_0, y_0, v_xi_0, v_eta_0);
