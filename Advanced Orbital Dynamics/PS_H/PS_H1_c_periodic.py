...
ic_table = (
    GT(df_po_table)
    .tab_header(
        title=md(f"Initial Conditions of the Periodic Orbit Found on Poincaré Map ({ps}, Lillian Shido)")
    )
    .cols_label(
        period="{{period}}<br>[nd]",
        x="{{x}}<br>[nd]",
...
po_chart = alt.Chart(df_po).mark_line(strokeWidth=1,clip=True).encode(
    ...
    width=400,
    height=400,
    title=[f"Periodic Orbit @ x={x0:.3f}[nd] for JC={JC}",f"({ps}, Lillian Shido)"]
)

po_chart_layer = alt.layer(po_chart)
po_chart_layer.save(f'{ps}_po.png', ppi=200)
